...
\end{equation}
The visual and geometric representation of these vehicle kinematics and movement updates is illustrated in Figure~\ref{fig:kinematics}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    \draw[gray, dashdotted, line width=1.0pt] ($(C0) + (195:2.2)$) -- ($(C) + (15:1.6)$) node[pos=0.0, above, black, font=\tiny, sloped] {Lane Centerline};
    
    % Position Vectors
    \draw[->, bluevector, line width=1.5pt] (0,0) -- (C0) node[midway, above left, bluevector, font=\small] {$\overrightarrow{p}_0$};
    \draw[->, bluevector!70!black, line width=1.5pt] (0,0) -- (C) node[midway, below right, bluevector!70!black, font=\small] {$\overrightarrow{p}$};
    
    % Displacement Vector s
    \draw[->, greenlane, line width=2.2pt] (C0) -- (C) node[midway, above, greenlane, font=\small, sloped] {$\Delta\overrightarrow{s} = \overrightarrow{d} \cdot (v \cdot \Delta t)$};
    ...
    \begin{scope}[shift={(C0)}, rotate=15]
      ...
      % Velocity v0
      \draw[->, -{Latex[scale=0.8]}, orangevector, line width=1.2pt] (0.6, 0.1) -- (1.5, 0.1) node[right, orangevector, font=\small] {$v_0$};
      % Acceleration a
      \draw[->, -{Latex[scale=0.8]}, redvector, line width=1.2pt] (0.6, -0.1) -- (1.2, -0.1) node[below, redvector, font=\small] {$\overrightarrow{a}$};
    \end{scope}
    ...
    \begin{scope}[shift={(C)}, rotate=15]
      ...
      \draw[->, -{Latex[scale=0.8]}, orangevector!80!black, line width=1.5pt] (0.6, 0) -- (1.9, 0) node[above, orangevector!80!black, font=\small] {$v = v_0 + a\Delta t$};
    \end{scope}
    
    % Centers
    \fill[bluevector] (C0) circle (2.2pt) node[below=20pt, black, font=\small] {$C_0(\overrightarrow{p}_0)$ at $t_0$};
    \fill[bluevector!70!black] (C) circle (2.2pt) node[below=20pt, black, font=\small] {$C(\overrightarrow{p})$ at $t_0 + \Delta t$};
    
    % Direction unit vector u (drawn starting from C0)
    \draw[->, -{Stealth}, black, line width=1.0pt] (C0) -- ($(C0) + (15:0.8)$) node[pos=0.35, above, black, font=\small] {$\overrightarrow{d}$};
...
\subsection{Current Car Acceleration}
...
\end{equation}
where $R_{\min}$ is the predefined minimum junction radius, and $\delta_{\text{marking}}$ is the buffer offset for road markings. The dynamic scaling behavior and its relationship with the maximum road half-width are geometrically illustrated in Figure~\ref{fig:junction_radius}.
...
\end{equation}
This formulation ensures that the outer edges of a multi-lane road intersect the circular boundary of the junction exactly, preserving visual and physical coherence, as diagrammed in Figure~\ref{fig:pythagorean_boundary}.
...
\end{equation}
For the $i$-th lane of the left-hand way ($i \in [0, N_l - 1]$), the lane start point $P_{\text{start}, i}$ and end point $P_{\text{end}, i}$ are generated using a scale factor $j = 2i + 1$:
\begin{align}
...
\begin{equation}
    \overrightarrow{u}_{\text{trans}} = R_{90^\circ}(\overrightarrow{u}) = \begin{pmatrix} -u_y \\ u_x \end{pmatrix}
\end{equation}
For the $i$-th lane of the right-hand way ($i \in [0, N_r - 1]$), the coordinates are generated as:
\begin{align}
...
\end{align}
This vector framework allows for the generation of parallel multi-lane systems for any arbitrary road angle, with the coordinate offsets and transformation vectors illustrated in Figure~\ref{fig:parallel_lanes}.
...
\end{align}
where ${x_{\min} = \min(x_1, x_2)}$, ${x_{\max} = \max(x_1, x_2)}$ (and similarly for segment $B$ denoted by primed limits). This precision padding guarantees stable collision detection near intersection junctions. Figure~\ref{fig:cramer_intersection} displays the geometric representation of these intersecting line segments and their respective bounding boxes.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      >=Latex,
...
\begin{enumerate}
    ...
    \end{equation}
\end{enumerate}
% The combined vector trajectory, containing both the longitudinal and lateral displacement vectors during a typical lane change maneuver, is shown in Figure~\ref{fig:lane_change_vector}.
\begin{figure}[H]
  \centering
  \begin{tikzpicture}[
      >=Latex,
...
\end{equation}
This affine transformation ensures spatial consistency between the logical map and the JavaFX rendering canvas during scaling operations, as shown in Figure~\ref{fig:zoom_math}.
...
\begin{figure}[H]
  ...
  \begin{tikzpicture}[
    ...
    % Draw Grid
    \draw[step=1.0, black!30, very thin] (-0.5, -1.5) grid (8.5, 5);
    
    % Draw Content Area (Logical Map)
    \draw[draw=black!60, fill=black!2, line width=1.2pt] (0,0) rectangle (8,4.5);
    \node[above right, black!75, font=\bfseries] at (0, 4.5) {Logical Map Content ($W_{\text{content}} \times H_{\text{content}}$)};
    ...
    \node[bluevector, font=\bfseries] at (3.75, 3.5) {Visible Viewport};
    
    % Viewport Width Dimension (drawn inside viewport)
    \draw[|<->|, bluevector!80!black, thin] (2, 1.8) -- (5.5, 1.8) node[midway, fill=bluevector!8, inner sep=1pt, font=\small] {$W_{\text{viewport}}$};
    ...
    \fill[redvector] (P) circle (2.2pt) node[above left=2pt, redvector, font=\bfseries\small, align=center] {Cursor \\ $(X_{\mathrm{mouse}}, Y_{\mathrm{mouse}})$};
    ...
    \draw[|<->|, black, thin] (0, -0.4) -- (2, -0.4) node[midway, fill=white, font=\small] {$X_{\text{viewport}}$};
    \draw[|<->|, bluevector, thin] (2, -0.4) -- (4.2, -0.4) node[midway, fill=white, inner sep=0pt, font=\small] {$\dfrac{X_{\text{mouse}}}{S}$};
    
    % Level 2: Target coordinate in logical Content
    \draw[|<->|, redvector, thin] (0, -1.0) -- (4.2, -1.0) node[midway, fill=white, inner sep=0pt, font=\small] {$X_{\text{target}} = X_{\text{viewport}} + \dfrac{X_{\text{mouse}}}{S}$};
    
    % Level 3: Content Width
    \draw[|<->|, black, thin] (0, -1.6) -- (8, -1.6) node[midway, fill=white, font=\small] {Total Content Width $W_{\text{content}}$};
...
\end{equation}
The use of the two-argument arctangent function, $\operatorname{atan2}(y, x)$, is mathematically critical for microscopic traffic visualization compared to the standard single-argument arctangent function $\arctan\left(\frac{y}{x}\right)$ \cite{giaitichToan}. 

Historically, the standard $\arctan(u)$ function only yields values within the restricted interval $\left(-\frac{\pi}{2}, \frac{\pi}{2}\right)$, which corresponds to the first and fourth quadrants of the Cartesian plane. Consequently, a single ratio $u = \frac{y}{x}$ is fundamentally ambiguous because it loses the individual signs of the coordinate components. For example:
\begin{equation}
...
\begin{equation}
    ...
    \end{cases}
\end{equation}
Furthermore, $\operatorname{atan2}$ robustly eliminates the mathematical singularity of division by zero that occurs when a vehicle moves purely along the vertical axis (where $d_x = 0$). By bypassing the explicit division $\frac{y}{x}$, the algorithm guarantees that vertical orientation transitions are perfectly stable. 

Once the angle is computed in radians, it is scaled by $\frac{180}{\pi}$ to convert it to degrees, which is the native rotation format for the JavaFX canvas coordinate system. The rendering engine then applies an affine translation to the center point of the rectangle, mapping the logical position $(p_x, p_y)$ directly to the JavaFX window coordinates. The step-by-step translation and rotation affine transformations applied during the vehicle rendering process are visualized in Figure~\ref{fig:rendering_transforms}.
