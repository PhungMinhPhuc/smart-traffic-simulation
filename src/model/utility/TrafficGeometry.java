...
// if (!isPointOnLine(x, y, x1, y1, x2, y2)) return null;
// if (!isPointOnLine(x, y, x3, y3, x4, y4)) return null;
...
// // If projections do not overlap on any axis, the rectangles do not
// intersect.
...
// private static TrafficPoint[] buildCorners(TrafficPoint start, TrafficPoint
// end, double halfWidth) {
...
// TrafficVector normal = new TrafficVector(-unit.getY(),
// unit.getX()).scale(halfWidth);

// // Return the 4 corners in clockwise order.
// return new TrafficPoint[] {
// start.moveBy(normal),
// end.moveBy(normal),
// end.moveBy(normal.scale(-1)),
// start.moveBy(normal.scale(-1))
// };
// }
