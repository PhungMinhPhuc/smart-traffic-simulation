...
public class MainSceneController {
	...
	Line previewLine = null; // line to show the road being dragged out when adding a new road by dragging
								// from start node to end node
	TrafficNode startNode = null; // placeHolder for TrafficNode for event handlers when adding or removing roads
									// and nodes
	...
	public void addNewRoad(ActionEvent event) {
		addingRoad = true;
		trafficMapWrapper.setPannable(false); // disable panning while adding road to avoid conflicts with dragging to
												// add road
		...
		mapLayer.setOnMousePressed((e) -> {
			if (!addingRoad)
				return; // not in adding road mode

			Point2D clickedPoint = trafficMapWrapper.getContent().sceneToLocal(e.getSceneX(), e.getSceneY());
			TrafficPoint localPoint = new TrafficPoint(clickedPoint.getX(), clickedPoint.getY()); // convert to custom
																									// TrafficPoint
		...
		mapLayer.setOnMouseDragged((e) -> {
			if (!addingRoad)
				return; // not in adding road mode
			if (previewLine == null)
				return; // not started dragging from a node
			...
			TrafficPoint localPoint = new TrafficPoint(draggedPoint.getX(), draggedPoint.getY()); // convert to custom
																									// TrafficPoint
		...
		mapLayer.setOnMouseReleased((e) -> {
			...
			TrafficPoint localPoint = new TrafficPoint(releasedPoint.getX(), releasedPoint.getY()); // convert to custom
																									// TrafficPoint
